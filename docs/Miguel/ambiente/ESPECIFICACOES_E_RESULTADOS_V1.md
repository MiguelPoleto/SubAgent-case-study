...
No inventário inicial (F0), as duas VMs tinham a mesma configuração efetiva para o agente principal (id real `crestodian`, ver REGISTRO_OPERACIONAL_V1.md OPS-015): opera diretamente no host, com sandbox desativado e sem containers de sandbox em execução. A simetria entre as VMs não é mais garantida desde a atualização do servidor: o **servidor** também tem o agente de teste `lab-test` (ver seção 6) e está em OpenClaw 2026.8.2, enquanto o **cliente** permanece só com a instalação padrão em 2026.7.1-2.

**Atualização (pós-migração para 2026.8.2, ver REGISTRO_OPERACIONAL_V1.md OPS-022 a OPS-027):** o id `crestodian` **não é mais um agente invocável** no servidor (`Unknown agent id "crestodian"`) — só resta um registro órfão dele no documento de aprovações, sem entrada correspondente em `agents.entries`. Decisão pendente do Miguel: recriar um agente de uso cotidiano sem restrições, ou seguir sem isso por enquanto. Além disso, a política de execução do `lab-test` deixou de ser `security: "deny"` e passou para `security: "full"` — foi necessário para o harness do modelo GPT-5-family (`openai/gpt-5.6-sol`, via Codex) conseguir processar qualquer turno, mesmo sem o agente pedir para executar algo. A partir de agora, a proteção efetiva do `lab-test` depende inteiramente de `tools.deny` + sandbox Docker (rede `none`, rootfs somente leitura, capabilities removidas via `capDrop: ["ALL"]`), **não mais de uma segunda camada independente de exec-approvals**. Isso deve ser reavaliado antes de rodar o `lab-test` contra conteúdo adversarial real (o EXP-001 ainda não foi executado).

**[CRÍTICO] Descoberta de projeto (OPS-027):** OpenClaw 2026.8.2 **ignora completamente o campo `workspace` na config do agente** e usa `~/.openclaw/agents/<agent-id>/agent/` como diretório de trabalho real. Isso é um comportamento não documentado ou um bug da versão 2026.8.2. Workaround confirmado: após configurar o agente, criar symlinks de `~/lab-test-workspace/*` para `~/.openclaw/agents/lab-test/agent/` via `ln -s /home/pinglab/lab-test-workspace/* ~/.openclaw/agents/lab-test/agent/ 2>/dev/null || true`. A partir daí, arquivos criados/editados em `~/lab-test-workspace/` aparecem automaticamente para o agente sem necessidade de cópias manuais.
